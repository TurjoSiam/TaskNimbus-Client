...
const UpdateTaskModal = ({ task, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.put(`${api_url}/updateTask/${task._id}`, {
        title,
        description,
        category,
      });
      if (data.modifiedCount) {
        refetch();
        onClose(); // Close the modal via prop function
        Swal.fire({
          icon: "success",
      ...
      <div className="modal-box text-black">
        <div className="w-full flex justify-between items-stretch text-black border-b border-[#d1d1d1]">
          ...
          <form method="dialog">
            <button
              type="button"
              onClick={onClose}  // Close modal using prop function
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4">
          ...
          {/* Task Description */}
          <div className="w-full md:w-[100%]">
            <label htmlFor="description" className="font-[500] text-[15px] text-text">
              Task Description
            </label>
            <textarea
              name="description"
          ...
          {/* Task Category */}
          <div className="w-full md:w-[100%]">
            <label htmlFor="category" className="font-[500] text-[15px] text-text">
              Update Category
            </label>
            <select
              value={selectedItem}
