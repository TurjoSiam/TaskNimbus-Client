    ...
    const onDragEnd = useCallback(
        (result) => {
            ...
            setTaskData((prev) => {
                ...
                // Ensure sourceCategory and destCategory exist
                if (!updatedTasks[sourceCategory] || !updatedTasks[destCategory]) return prev;

                // Remove task from source
                const [movedTask] = updatedTasks[sourceCategory].splice(source.index, 1);

                // If the moved task doesn't exist, return the previous state
        ...
        async (id) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const { data } = await axios.delete(`${api_url}/deleteTask/${id}`);
                                        ...
                                        {(provided) => (
                                            ...
                                            >
                                                ...
                                                <div className="mt-4 flex items-start gap-4 w-full">
                                                    {/* Update Btn */}
                                                    <button
                                                        onClick={() => handleOpenModal(task)}  // Open modal on click
